...
Comfy Registry版には、すぐ試せるサンプルとしてSFW / 一般向けのタグカタログとタグセットを収録しています。ローカルやGitHubのデータを差し替えたり、自分用のJSONを読み込んだりすることで、用途に合わせて内容を拡張できます。
...
そのため、Registry同梱版の既定カタログはSFW版のサンプルデータです。
...
Comfy Registryへ公開するパッケージでは、公開ワークフロー内で `data/sfw_tag_sets.json` を `data/tag_sets.json` として差し替えます。
そのため、Registry同梱版の既定タグセットはSFW / 一般向けのサンプルデータです。
