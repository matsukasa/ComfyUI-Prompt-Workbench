...
- タグセットを使い、服装、背景のイメージ、髪型などのまとまったタグを一括追加できます。
...
タグセットは、よく使うタグのまとまりを一度に追加するための機能です。
初期状態では、服装、背景のイメージ、髪型のセットを用意しています。

`data/tag_sets.json` がある場合は、そのファイルの内容を読み込みます。
ファイルがない場合でも、内蔵の最小タグセットを使えるようにしています。
