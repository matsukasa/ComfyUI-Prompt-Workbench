...
| データ | 大分類 | 中分類 | 小分類 | 件数 |
| --- | ---: | ---: | ---: | ---: |
| タグカタログ `data/tag_catalog.json` | 8 | 33 | 139 | 2,922タグ |
| タグセット `data/tag_sets.json` | 6 | 20 | 50 | 632セット |
...
タグセットは、内容を個別のタグに展開して追加します。追加後の有効／無効、並べ替え、重み調整も個々のタグに対して行います。
...
上限は2,000セット、500カテゴリー、1セットあたり100タグです。上限超過はエラーとして表示し、途中まで切り詰めて読み込むことはありません。古いファイルに同じセットIDが複数ある場合は、先頭のIDを維持し、後続に `:duplicate:2` などを付けて区別します。既存のお気に入りは先頭のセットに対応し、別のセットには自動登録しません。
...
お気に入りはComfyUIユーザーディレクトリにも共有保存します。複数のWorkbenchからの追加・解除は差分で保存され、同じ画面の各ノードに反映されます。別ウィンドウでの変更は約5秒ごとに確認します。共有ファイルがない初回だけワークフロー内のお気に入りを移行し、それ以降は共有データを正として、古いワークフローを開いても解除済み項目を復活させません。状態JSONを明示的にImportした場合は、そこに含まれるお気に入りを共有一覧へ追加します。
...
一括翻訳は100件以下かつリクエストのサイズ上限内に分割し、進捗を表示します。翻訳中にタグを移動しても元のタグへ結果を適用し、削除・編集されたタグへの古い結果は破棄します。翻訳後の重み変更も出力へ反映し、タグを直接編集した場合は古い翻訳を解除します。
